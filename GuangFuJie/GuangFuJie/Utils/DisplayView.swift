...
extension DisplayView{
    ...
    func imgsPrepare(imgs: [String], isLocal: Bool){
        ...
        for i in 0..<imgs.count {
            ...
            imgV.addGestureRecognizer(UITapGestureRecognizer(target: self, action: #selector(self.tapAction(_:))))
            imgV.tag = i
            if isLocal {
                var image = UIImage(named: imgs[i])
                if (image == nil) {
                    image = UIImage.init(data: NSData.init(contentsOfURL: NSURL.init(string: imgs[i])!)!)
                }
            }else{
